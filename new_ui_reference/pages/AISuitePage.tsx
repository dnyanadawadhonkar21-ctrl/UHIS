...
export default function AISuitePage() {
  ...
  const [ddiResult, setDdiResult] = useState<typeof DDI_RESPONSES[0] | null>(null);
  const [ocrFile, setOcrFile] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);

  const simulateAI = async (fn: () => void) => {
    setLoading(true);
            ...
            <InstrumentPanel title="Drug-Drug Interaction Checker" subtitle="POLYPHARMACY SAFETY" channel="warning">
              ...
              <div style={{ marginTop: "1.5rem", paddingTop: "1.25rem", borderTop: "1px solid var(--color-border)" }}>
                ...
                {DDI_RESPONSES.map((d, i) => (
                  <div
                    key={i}
                    style={{ padding: "0.4rem 0", borderBottom: "1px solid var(--color-border)", display: "flex", justifyContent: "space-between" }}
                  >
          ...
          <div className="fade-in">
            <InstrumentPanel title="Medical OCR Scanner" subtitle="DOCUMENT DIGITISATION" channel="purple">
              ...
              {ocrFile && !loading && (
                <div
                  style={{
                    background: "var(--color-signal-purple-bg)",
                    border: "1px solid var(--color-signal-purple-border)",
