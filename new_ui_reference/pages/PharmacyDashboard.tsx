...
export default function PharmacyDashboard() {
  ...
  const [selectedRx, setSelectedRx] = useState(prescriptionQueue[0]);
  const [dispensed, setDispensed] = useState<string[]>(["RX-2024-0711-0038"]);
        ...
        {activeTab === "queue" && (
          <div className="fade-in">
            {prescriptionQueue.map((rx) => {
              const isDispensed = dispensed.includes(rx.rxId);
              return (
                <div
                  key={rx.rxId}
                ...
                >
                  <div className="panel-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      ...
                      <div className="type-heading">{rx.patient}</div>
                    </div>
                    <div style={{ display: "flex", gap: "0.75rem", alignItems: "center" }}>
                      ...
                      {!isDispensed && (
                        <Button
                          size="sm"
                          onClick={() => { setDispensed((d) => [...d, rx.rxId]); toast.success(`Rx ${rx.rxId} dispensed successfully.`); }}
                        >
                          DISPENSE ALL →
                        </Button>
                      )}
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", padding: "0.5rem 1.25rem", background: "var(--color-surface)", borderBottom: "1px solid var(--color-border)" }}>
                      <span className="type-label" style={{ color: "var(--color-ink-secondary)" }}>PRESCRIBED BY: {rx.doctor}</span>
                      <span className="type-label" style={{ color: "var(--color-ink-secondary)" }}>ABHA: {rx.abhaId}</span>
                    </div>
                    {rx.items.map((item, i) => (
                      <div
                        key={i}
