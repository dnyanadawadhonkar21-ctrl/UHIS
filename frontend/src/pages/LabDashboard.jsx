...
export default function LabDashboard() {
  ...
  const filtered = filter === "ALL" ? labOrders
    : labOrders.filter((o) => (o.status || '').replace("-", " ").toUpperCase() === filter);

  const stats = {
    total: labOrders.length,
    pending: labOrders.filter((o) => o.status === "sample-received").length,
  ...
  return (
    <AppLayout tabs={TABS} activeTab={activeTab} onTabChange={setActiveTab}>
        ...
        {activeTab === "orders" && (
          <div className="fade-in">
            <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.25rem", flexWrap: "wrap" }}>
              {["ALL", "SAMPLE RECEIVED", "PROCESSING", "COMPLETED"].map((f) => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
            ...
            {filtered.map((o) => (
              <div
                key={o.id}
                className={`instrument-panel channel-${STATUS_SIGNAL[o.status] || "info"}`}
                ...
                <div style={{ padding: "1rem 1.25rem", display: "flex", justifyContent: "space-between", gap: "1rem", flexWrap: "wrap", alignItems: "center" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", gap: "0.75rem", alignItems: "baseline", marginBottom: "0.3rem" }}>
                      ...
                      <StatusCode status={PRIORITY_SIGNAL[o.priority] || "info"} label={(o.priority || "ROUTINE").toUpperCase()} />
                    </div>
                    <div className="type-value" style={{ color: "var(--color-ink)", marginBottom: "0.3rem" }}>{o.testName || o.test}</div>
                    <div style={{ display: "flex", gap: "1.25rem", flexWrap: "wrap" }}>
                      <span className="type-label" style={{ color: "var(--color-ink-secondary)" }}>PATIENT: {o.patientName || o.patient}</span>
                      <span className="type-label" style={{ color: "var(--color-ink-secondary)" }}>COLLECTED: {o.collectedAt || "08:30 AM"}</span>
                      <span className="type-label" style={{ color: "var(--color-ink-secondary)" }}>EXPECTED: {o.expectedBy || "05:00 PM"}</span>
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                    <StatusCode status={STATUS_SIGNAL[o.status] || "info"} label={(o.status || "").replace("-", " ").toUpperCase()} />
                    {o.status !== "completed" && (
                      <Button
                        size="sm"
                        onClick={() => { setSelectedOrder(o); setResultOpen(true); }}
                      >
                        {o.status === "sample-received" ? "PROCESS" : "ENTER RESULT"}
                      </Button>
                    )}
                  </div>
                </div>
            ...
            <InstrumentPanel title="Manual Result Entry" subtitle="RESULT ENTRY FORM" channel="info">
              <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                  <PrecisionInput label="Order ID" placeholder="LO-9901" defaultValue="LO-9901" />
                  <PrecisionInput label="Patient ABHA ID / Name" placeholder="91-4782-3391-6284" defaultValue="Rahul Verma" />
                </div>
                <PrecisionInput label="Test Name" placeholder="HbA1c, CBC, Thyroid Profile..." defaultValue="Thyroid Profile (T3, T4, TSH)" />
                <PrecisionInput label="Result Value" placeholder="e.g. TSH: 2.4 uIU/mL (Normal), Free T4: 1.2 ng/dL" />
                <PrecisionInput label="Reference Range" placeholder="TSH: 0.4 - 4.0 uIU/mL" defaultValue="TSH: 0.4 - 4.0 uIU/mL" />
                <div>
            ...
            <InstrumentPanel title="Report Dispatch Queue" subtitle="COMPLETED REPORTS" channel="normal">
              {labOrders.filter((o) => o.status === "completed").map((o) => (
                <div key={o.id} className="data-row">
                  <div>
                    <div className="type-value" style={{ color: "var(--color-ink)", fontSize: "0.85rem" }}>{o.testName || o.test}</div>
                    <div className="type-micro" style={{ color: "var(--color-ink-secondary)" }}>{o.patientName || o.patient} · Collected: {o.collectedAt}</div>
                  </div>
                  <div style={{ display: "flex", gap: "0.5rem" }}>
                    <Button size="sm" variant="secondary" onClick={() => toast.info(`PDF dispatched to ${o.patientName || o.patient}.`)}>DISPATCH PDF</Button>
                  </div>
                </div>
              ))}
            </InstrumentPanel>
      ...
      <Modal isOpen={resultOpen} onClose={() => setResultOpen(false)} title="Enter Test Result" subtitle="RESULT ENTRY">
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div className="type-value" style={{ color: "var(--color-ink)" }}>{selectedOrder.testName || selectedOrder.test}</div>
          <div className="type-micro" style={{ color: "var(--color-ink-secondary)" }}>Patient: {selectedOrder.patientName || selectedOrder.patient} · {selectedOrder.id}</div>
